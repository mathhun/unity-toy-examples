		...
		else if (data[idx] == CMD_TRX)
		{
			...
			if (data[idx] == TRX_START)
			{
				...
				if (socket.inTransaction)
				{
					...
					{
						fs.unlink (socket.targets[i].from);
					}
				}
			...
			else if (data[idx] == TRX_END)
			{
				...
				for (var i = 0 ; i < socket.targets.length ; i++)
				{
					...
					MoveFile (socket.targets[i].from, socket.targets[i].to, socket.targets[i].size);
				}
...
function MoveFile (from, to, size)
{
	fs.stat (to, function (statsErr, stats)
	{
		...
		{			
			// When the replace fails. We just delete the temp file. The size of the cache has not changed.				
			if (err)
			{
				log (DBG, "Failed to move file in place " + from + " to " + to + " (" + err + ")");				
				fs.unlink (from);
			}
